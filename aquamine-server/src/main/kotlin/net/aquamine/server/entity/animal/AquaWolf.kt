...
class AquaWolf(world: AquaWorld) : AquaTamable(world), Wolf, Neutral {

    override val type: AquaEntityType<AquaWolf>
        get() = AquaEntityTypes.WOLF
    override val serializer: EntitySerializer<AquaWolf>
        get() = WolfSerializer
    ...
    override var isTamed: Boolean
        ...
        set(value) {
            super.isTamed = value
            if (value) {
                getAttribute(AquaAttributeTypes.MAX_HEALTH)?.baseValue = TAMED_HEALTH
                health = TAMED_HEALTH.toFloat()
            } else {
                getAttribute(AquaAttributeTypes.MAX_HEALTH)?.baseValue = UNTAMED_HEALTH
            }
            getAttribute(AquaAttributeTypes.ATTACK_DAMAGE)?.baseValue = TAME_UPDATE_ATTACK_DAMAGE
        }
