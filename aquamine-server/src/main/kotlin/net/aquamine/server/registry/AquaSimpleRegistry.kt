...
/**
 * A simple registry implementation that provides all of the required functionality of AquaRegistry.
 */
open class AquaSimpleRegistry<T> protected constructor(override val key: ResourceKey<out Registry<T>>, intrusive: Boolean) : WritableRegistry<T> {
    ...
    override fun tags(): Collection<TagSet<T>> {
        val transformed = Maps.transformValues(tags) { AquaTagSet(this, it) }
    ...
    override fun freeze(): AquaRegistry<T> {
    ...
    companion object {
        ...
        @JvmStatic
        fun <T> standard(key: ResourceKey<out Registry<T>>): AquaSimpleRegistry<T> = AquaSimpleRegistry(key, false)

        @JvmStatic
        fun <T> intrusive(key: ResourceKey<out Registry<T>>): AquaSimpleRegistry<T> = AquaSimpleRegistry(key, true)
    }
}
