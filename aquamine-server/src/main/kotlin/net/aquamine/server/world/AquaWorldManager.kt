...
class AquaWorldManager(
    ...
    private val worldExecutor = ThreadPoolBuilder.create()
        ...
        .build()
    private val data = checkNotNull(dataSerializer.load(name())) { "You must provide an existing world for AquaMine!" }

    override val worlds: MutableMap<ResourceKey<World>, AquaWorld> = ConcurrentHashMap()
    override val default: AquaWorld
    ...
    override fun loadWorld(key: Key): CompletableFuture<AquaWorld?> {
        ...
        val dimensionType = AquaDynamicRegistries.DIMENSION_TYPE.get(key) as? AquaDimensionType
        ...
        return CompletableFuture.supplyAsync({
            ...
            AquaWorld(server, worldData, resourceKey, dimensionType, chunkLoader)
        }, worldExecutor)
    }
    ...
    private fun create() {
        val dimension = AquaDimensionTypes.OVERWORLD
        val world = AquaWorld(server, data, World.OVERWORLD, dimension, chunkLoader)
