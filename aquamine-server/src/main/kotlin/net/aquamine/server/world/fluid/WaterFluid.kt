...
abstract class WaterFluid : FlowingFluid() {
    ...
    override fun flowing(): AquaFluid = AquaFluids.FLOWING_WATER

    override fun source(): AquaFluid = AquaFluids.WATER

    override fun isSame(fluid: Fluid): Boolean = fluid === source() || fluid === flowing()

    override fun asBlock(state: AquaFluidState): AquaBlockState =
        AquaBlocks.WATER.defaultState.setProperty(LEVEL, calculateBlockLevel(state))
    ...
    class Source : WaterFluid() {
        ...
        override fun isSource(state: AquaFluidState): Boolean = true
    }
}
