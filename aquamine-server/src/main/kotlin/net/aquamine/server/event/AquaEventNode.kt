...
open class AquaEventNode<T : Event>(
    ...
    private val childrenSet = CopyOnWriteArraySet<AquaEventNode<T>>()

    @Volatile
    override var priority: Int = 0
    @Volatile
    private var parentNode: AquaEventNode<in T>? = null
    ...
    override fun addChild(node: EventNode<out T>) {
        synchronized(GLOBAL_CHILD_LOCK) {
            val child = node as AquaEventNode<out T>
            ...
            if (!childrenSet.add(child as AquaEventNode<T>)) return
    ...
    private fun invalidateEventsFor(node: AquaEventNode<in T>) {
    ...
    private class Handle<E : Event>(private val node: AquaEventNode<E>, private val eventType: Class<E>) {
    ...
    object Factory : EventNode.Factory {
        ...
        override fun <T : Event, V> create(name: String, filter: EventFilter<T, V>, predicate: BiPredicate<T, V?>?): EventNode<T> {
            @Suppress("UNCHECKED_CAST")
            return AquaEventNode(name, filter, predicate as? BiPredicate<T, Any?>)
        }
    }
